...
router.get("/api/articulos", async function (req, res, next) {
  ...
  if (req.query.Nombre != undefined && req.query.Nombre !== "") {
    where.Nombre = {
      //Se busca algun nobre parecido al parametro recibido
  ...
  //?? es un operador que devuelve el valor de su lado derecho cuando su lado izquierdo es nulo
  //Osea, si el usuario no pasa ningun parametro de pagina, se asume que quiere ver la primera
  ...
  const { count, rows } = await db.articulos.findAndCountAll({
    ...
    order: [["Nombre", "ASC"]],
    where,
    //offset se encarga de ocultar una cantidad dada de registros, si pagina abajo fuera 3 se ocultarian los primeros 20 registros
